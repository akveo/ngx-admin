...
export class BaCardBlurHelper {
  ...
  public getBodyBgImageSizes(): BgMetrics {
    const elemW: number = document.documentElement.clientWidth;
    const elemH: number = document.documentElement.clientHeight;
    if(elemW <= 640) return;
    const imgRatio: number = (this.image.height / this.image.width);       // original img ratio
    const containerRatio: number = (elemH / elemW);     // container ratio
  ...
  private _genBgImage(): void {
    ...
    const computedStyle: any = getComputedStyle(document.body.querySelector('main'), ':before');
